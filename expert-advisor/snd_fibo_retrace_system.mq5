...
void ActionBuyVolatile();
void ActionSellVolatile();
void ActionDelBuy();
void ActionDelSell();
void ActionLim236();
void ActionLim618();
...
void ActionBuyChoppy()      { PlaceRegimeOrders(true, false); }
void ActionSellChoppy()     { PlaceRegimeOrders(false, false); }
void ActionBuyVolatile()    { PlaceRegimeOrders(true, true); }
void ActionSellVolatile()   { PlaceRegimeOrders(false, true); }
void ActionDelBuy()         { DelPO(ORDER_TYPE_BUY_LIMIT); }
void ActionDelSell()        { DelPO(ORDER_TYPE_SELL_LIMIT); }
void ActionLim236()         { PlaceFiboDirLimit(1); }
void ActionLim618()         { PlaceFiboDirLimit(2); }
...
void HandleHotkey(const long key)
{
   ...
   else if(key == 86) ActionBuyVolatile();   // V
   else if(key == 69) ActionSellVolatile();  // E
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
{
   ...
   if(id == CHARTEVENT_OBJECT_CLICK)
   {
      ...
      else if(sparam == PREF+"BtnLim618") { ActionLim618(); ObjectSetInteger(0, PREF+"BtnLim618", OBJPROP_STATE, false); }
      ...
      else if(sparam == PREF+"DelBuy") { ActionDelBuy(); ObjectSetInteger(0, PREF+"DelBuy", OBJPROP_STATE, false); }
      else if(sparam == PREF+"DelSell") { ActionDelSell(); ObjectSetInteger(0, PREF+"DelSell", OBJPROP_STATE, false); }
...
void ApplyQuoteVisibility()
{
   int yCheck1 = IsQuoteVisible ? 75  : UI_OFFSCREEN;
   int yCheck2 = IsQuoteVisible ? 100 : UI_OFFSCREEN;
   int yCheck3 = IsQuoteVisible ? 125 : UI_OFFSCREEN;
   int yQuote  = IsQuoteVisible ? 150 : UI_OFFSCREEN;
...
void CreateCalcLotLabel()
{
   ...
   ObjectSetInteger(0, name, OBJPROP_YDISTANCE, UI_OFFSCREEN);
   ObjectSetInteger(0, name, OBJPROP_FONTSIZE, 9);
   ObjectSetString(0, name, OBJPROP_FONT, "Arial");
...
void RefreshRiskDisplay()
{
   if(g_fiboActive || IsFiboOnChart())
   {
      ...
      if(e382 > 0.0 && e50 > 0.0 && sl > 0.0 && MathAbs(e382 - sl) > 0.0)
      {
         ...
         if(ObjectFind(0, PREF+"Sell_Risk") >= 0)
            ObjectSetString(0, PREF+"Sell_Risk", OBJPROP_TEXT, DoubleToString(sellRisk, 2));
   ...
   if(ObjectFind(0, PREF+"Buy_Risk") >= 0)
      ObjectSetString(0, PREF+"Buy_Risk", OBJPROP_TEXT, "0.00");
   if(ObjectFind(0, PREF+"Sell_Risk") >= 0)
...
void CreateDashboard() {
   ...
   ObjectSetInteger(0, PREF+"Hide", OBJPROP_ZORDER, 20);
   ObjectSetInteger(0, PREF+"HideQuote", OBJPROP_ZORDER, 20);
   CreateLabel("Title", (PANEL_W / 2) + 20, HEADER_Y + 20, "SND Fibo Retrace", clrWhite);
   CreateObject("Panel", OBJ_RECTANGLE_LABEL, 0, 10, UI_Y, PANEL_W, PANEL_H, clrDarkSlateGray);
   
   CreateLabel("LblLayers", 20, UI_Y+26, "Layers", clrOrange);
   CreateEdit("InpLayers", 130, UI_Y+14, 110, 25, "1");
   CreateButton("BtnLotMode", 300, UI_Y+14, 90, 25, "Lot", clrDarkSlateGray, clrOrange);
   CreateEdit("InpLot", 410, UI_Y+14, 110, 25, "1.00");
   CreateCalcLotLabel();
   ApplyLotModeUI();

   CreateLabel("LB_Risk", 20, UI_Y+62, "Risk $", clrOrange);
   CreateEdit("Buy_Risk", 130, UI_Y+50, 110, 25, "0.00");
   CreateLabel("LS_Risk", 300, UI_Y+62, "Risk $", clrOrange);
   CreateEdit("Sell_Risk", 410, UI_Y+50, 110, 25, "0.00");
...
void SetPickZzSegMode(const bool on)
{
   ...
   if(on)
   {
      ObjectSetString(0, PREF+"BtnPickZzSeg", OBJPROP_TEXT, "Klik Segmen [Z]");
      ObjectSetInteger(0, PREF+"BtnPickZzSeg", OBJPROP_BGCOLOR, clrOrangeRed);
   }
   else
   {
      ObjectSetString(0, PREF+"BtnPickZzSeg", OBJPROP_TEXT, "Pilih ZZ [Z]");
...
double GetInputValue(string name)
{
   if(ObjectFind(0, PREF+name) < 0) return 0.0;
   return StringToDouble(ObjectGetString(0, PREF+name, OBJPROP_TEXT));
}
...
void UpdateInput(string name, double price)
{
   if(ObjectFind(0, PREF+name) < 0) return;
   ObjectSetString(0, PREF+name, OBJPROP_TEXT, DoubleToString(price, _Digits));
}
...
void CreateLabel(string name, int x, int y, string text, color clr)
{
   CreateObject(name, OBJ_LABEL, 0, x, y, 0, 0, clr);
   ObjectSetString(0, PREF+name, OBJPROP_TEXT, text);
   ObjectSetInteger(0, PREF+name, OBJPROP_ANCHOR, ANCHOR_LEFT);
   ObjectSetInteger(0, PREF+name, OBJPROP_FONTSIZE, 10);
}
...
int GetInitialY(string name) {
   ...
   if(name == PREF+"LblCalcLot") return UI_OFFSCREEN;
